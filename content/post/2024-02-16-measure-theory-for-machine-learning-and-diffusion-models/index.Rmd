...
Another great application of the change of variables theorem is being able to write expectations as we usually do in an undergraduate course in probability. Indeed, so far we have been writing the expectation of a random variable $\mathrm{X}:\Omega\to\Rbb$ as integrals over the sample space $\Omega$
$$
\Ebb[\Xrm] = \int_{\Omega} \Xrm(\omega) \Pbb(d\omega)
$$
but these look different from the usual definition of expectation that we are all familiar with
$$
\int_\Xsf x p(x) dx
$$
where $p(x)$ is the probability density function of the random variable $\Xrm$. This can be easily recovered
$$
