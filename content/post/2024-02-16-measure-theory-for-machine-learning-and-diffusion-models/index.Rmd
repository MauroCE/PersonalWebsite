...
# Topology and Borel Sigma algebras
...
Seems very abstract. On $\mathbb{R}$ there are many possible topologies that we could consider, but in most cases people are interested in the **standard topology on $\mathbb{R}$**, which we will see soon. A topology can contain many subsets and it may be awkward or difficult to describe exactly which topology we are referring to. Luckily, there is a way around this. If we have a **base**, i.e. a bunch of subsets $\mathsf{B}\subseteq 2^{\mathsf{X}}$, one can **generate** any element of the topology as a union of elements in $\mathsf{B}$. This makes being precise about which topology we are talking about, much simpler.
...
Again, lots of notation, but the idea is simple: any set of the topology can be written as a union of sets in the basis. Now, with topology out of the way, we can talk about **Borel sigma algebras** these are sigma algebras that are generated by a topology.
...
I have to admit, I struggle to find a valid intuition that is not rooted in mathematical details. However, one way I like to think about it, is that a topology gives you a notion of *closeness* of elements of $\mathsf{X}$, while a sigma algebra gives you the notion of relative sizes of subsets of $\mathsf{X}$. The Borel sigma algebra connects these two concepts. Being generated by the topology, means that every subset in the topology is also an element of the sigma algebra, meaning that we now have both notions!

The topology on $\mathbb{R}$ that gives rise to the Borel sigma algebra is called the standard topology on $\mathbb{R}$. This topology is generated by all open intervals $(a, b)$
$$
\mathsf{B} = \left\{\left\{x\in\mathbb{R}\,:\, a < x < b\right\} \, :\, a, b\in\mathbb{R} \text{ with } a < b\right\}.
$$
The topology generated by this basis then generates the Borel sigma algebra $\mathbb{B}(\mathbb{R})$. What about $\mathbb{R}^n$? We basically iterate this process for each copy of $\mathbb{R}$. Given two topological spaces with bases, we can create a **product** topological space whose base is the product of bases (I promise, almost done with topology).

> **Product Topological Space**: Let $(\mathsf{X}, \tau_{\mathsf{X}})$ and $(\mathsf{Y}, \tau_{\mathsf{Y}})$ be two topological spaces with bases $\mathsf{B}_\mathsf{X}$ and $\mathsf{B}_\mathsf{Y}$ respectively. Let $\mathsf{X}\times\mathsf{Y}$ be their Cartesian product
$$
\mathsf{X}\times\mathsf{Y} := \left\{(x, y) \,:\, x\in\mathsf{X}, y\in\mathsf{Y}\right\}.
$$
Then, we can generate a topology on this product space, denoted $\tau_{\mathsf{X}\times\mathsf{Y}}$ and this is generated from the base of 

Okay, first of all $(x, y)$ is **not** an interval, it is just a pair. 

# Lebesgue Measure
Remember the integrals that we used to solve in high-school?
$$
\int f(x) dx
$$
The symbol $dx$ is actually representing a very special measure: the Lebesgue measure. Indeed, in more theoretically-oriented textbooks, one would probably write $\text{Leb}(dx)$ or something like that. All you have to know, is that the Lebesgue is $\sigma$-finite and that when we are working on $(\mathbb{R}, \mathcal{B}(\mathbb{R}))$ we usually equip this measurable space with the Lebesgue measure on it, and typically call it $\text{Leb}_{\mathbb{R}}$. This measure represents our native way of thinking about length, and a good way to convince ourselves of that, is knowing that the size of an interval $[a, b]$ with $a < b$ is indeed
$$
\text{Leb}_\mathbb{R}([a, b]) = b - a.
$$
The Lebesgue measure is then defined also for $(\mathbb{R}^n, \mathcal{B}(\mathbb{R}^n))$ as a product of "univariate" Lebesgue measures.
...
Very common in machine learning, is a normal density function with mean $\mu$ and scale $\sigma > 0$
$$
p(x; \mu, \sigma) = \frac{1}{\sqrt{2\pi}\sigma}\exp\left(-\frac{(x - \mu)^2}{2\sigma^2}\right),\qquad\qquad x\in\mathbb{R}
$$
We often say that a random variable $\mathrm{X}$ has a Gaussian distribution and then immediately think of the expression above. What is this expression measure-theoretic language? It is not a distribution. A probability density function tells us how much the distribution of $\mathrm{X}$, denoted $\mathbb{P}^{\mathrm{X}}$ changes against another measure. What is this "other measure" and why is it never mentioned? It is rarely mentioned because typically it is always the same: the Lebesgue measure. 
