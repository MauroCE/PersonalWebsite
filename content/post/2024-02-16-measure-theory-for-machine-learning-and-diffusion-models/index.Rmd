...
Finally, very often people will write $\Pbb(\Xrm\in\Asf)$ rather than $\Pbb^\Xrm(\Asf)$. It is straightforward to verify that they are indeed the same thing, once we define the notation $\Xrm\in\Asf$. 

> **Cute notation for $\Pbb^\Xrm(\Asf)$**: Let $(\Omega, \Fcal, \Pbb)$ be a probability space, $(\Xsf, \Bcal(\Xsf), \|\cdot\|_\Xsf)$ be a Banach-Borel space and $\Xrm:\Omega\to\Xsf$ be a random variable in $L^1(\Omega, \Fcal, \Pbb; \Xsf)$. Then we can write
$$
\Pbb^\Xrm(\Asf) = \Pbb^\Xrm(1_\Asf) = \Pbb(1_\Asf \circ \Xrm)
$$
and by definition the composition of the indicator function and the random variable is
$$
1_\Asf(\Xrm(\omega)) = \begin{cases}
    1 & \Xrm(\omega) \in \Asf \\
    0 & \Xrm(\omega) \notin \Asf 
\end{cases}
$$
and then we can **define** the set $\{\Xrm \in \Asf\} := \left\{\omega\in\Omega\,:\, \Xrm(\omega)\in \Asf\right\}$, meaning indeed $\Pbb^\Xrm(\Asf) = \Pbb(\{\Xrm\in\Asf\})$ and we typically drop the parenthesis and simply say $\Pbb(\Xrm\in\Asf)$.
