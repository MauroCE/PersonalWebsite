...
$$
-\frac{1}{2}\vx^\top \vSigma^{-1}\vx = -\frac{1}{2}\sum_{k=1}^d \sum_{j=1}^d x_k\Sigma_{kj}x_j
$$
similarly, the following expression can be written in the same way
$$
\begin{align}
\text{tr}\left[-\frac{1}{2}\vSigma\vx\vx^\top\right] &= 
...
\end{pmatrix}
\right]\\
&= -\frac{1}{2}\sum_{k=1}^d\sum_{j=1}^d x_{k}\Sigma_{kj}x_j
\end{align}
$$
...
$$
\begin{align}
\left\langle -\frac{1}{2}\vSigma, \vx\vx^\top\right\rangle_F 
&= \text{tr}\left(-\frac{1}{2}\vSigma\vx\vx^\top\right) \\
&= \text{vec}\left(-\frac{1}{2}\vSigma\right)^\top\text{vec}\left(\vx\vx^\top\right)
\end{align}
$$
...
$$
f(\vx) = \exp\left\{\langle \vSigma^{-1}\vmu, \vx\rangle + \left\langle \text{vec}\left(-\frac{1}{2}\vSigma\right), \text{vec}\left(\vx\vx^\top\right)\right\rangle -\frac{1}{2}\left[d\log2\pi + \log|\vSigma| +\vmu^\top\vSigma^{-1}\vmu\right]\right\}
$$
